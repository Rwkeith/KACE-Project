...
// unused, but maybe later.  we can't actually execute code in fltmgr.sys that would depend on kernel functionality.
// fltmgr.sys has PAGE_NO_ACCESS anyways.
template <typename... Params>
...
void FilterModuleInformation(uint32_t  SystemInformationClass,
	...
	for (int i = 0; i < loadedmodules->NumberOfModules; i++)
	{
		...
		if (mapped_module)
		{
			Logger::Log("\tPatching %s base from %llx to %llx\n",
						modulename,
						(PVOID)loadedmodules->Modules[i].ImageBase,
...
void FilterModuleInformationx4D(uint32_t  SystemInformationClass,
								uintptr_t SystemInformation,
								ULONG	  SystemInformationLength,
								PULONG	  ReturnLength)
{
		...
		while ((SizeRead + sizeof(_SYSTEM_MODULE_EX)) <= *ReturnLength)
		{
			...
			if (mapped_module)
			{
				Logger::Log("\tPatching %s base from %llx to %llx\n",
							modulename,
							pMods->ImageBase,
							mapped_module->GetMappedImageBase());
...
NTSTATUS FilterSystemProcessInformation(uint32_t  SystemInformationClass,
										uintptr_t SystemInformation,
										ULONG	  SystemInformationLength,
										PULONG	  ReturnLength)
{
...
NTSTATUS Filter0x5a(uint32_t  SystemInformationClass,
					uintptr_t SystemInformation,
					ULONG	  SystemInformationLength,
					PULONG	  ReturnLength)
{
	...
	if (SystemInformationLength)
	{
		switch (SystemInformationClass)
		{
			case 0xB:  // SystemModuleInformation
				FilterModuleInformation(
					SystemInformationClass, SystemInformation, SystemInformationLength, ReturnLength);
				break;

			case 0x4D:	// SystemModuleInformation
				FilterModuleInformationx4D(
					SystemInformationClass, SystemInformation, SystemInformationLength, ReturnLength);
				break;
			case 0x5a:
				...
				break;
			case 0x5:
				FilterSystemProcessInformation(
					SystemInformationClass, SystemInformation, SystemInformationLength, ReturnLength);
				break;
			default:
				break;
...
NTSTATUS h_IoCreateFileEx(PHANDLE			 FileHandle,
						  ...
						  void*				 DriverContext)
{
	...
	auto ret = __NtRoutine("NtCreateFile",
						   FileHandle,
						   DesiredAccess,
						   ObjectAttributes,
						   IoStatusBlock,
...
EXCEPTION_DISPOSITION _c_exception(_EXCEPTION_RECORD*	ExceptionRecord,
								   void*				EstablisherFrame,
								   _CONTEXT*			ContextRecord,
								   _DISPATCHER_CONTEXT* DispatcherContext)
{
	return (EXCEPTION_DISPOSITION)__NtRoutine(
		"__C_specific_handler", ExceptionRecord, EstablisherFrame, ContextRecord, DispatcherContext);
}

NTSTATUS h_RtlMultiByteToUnicodeN(PWCH		  UnicodeString,
								  ...
								  ULONG		  BytesInMultiByteString)
{
	...
	return __NtRoutine("RtlMultiByteToUnicodeN",
					   UnicodeString,
...
NTSTATUS h_NtCreateFile(PHANDLE			   FileHandle,
						ACCESS_MASK		   DesiredAccess,
						OBJECT_ATTRIBUTES* ObjectAttributes,
						PVOID			   IoStatusBlock,
						PLARGE_INTEGER	   AllocationSize,
...
NTSTATUS h_NtReadFile(HANDLE		 FileHandle,
					  HANDLE		 Event,
					  PVOID			 ApcRoutine,
					  PVOID			 ApcContext,
					  PVOID			 IoStatusBlock,
								  ...
								  FILE_INFORMATION_CLASS FileInformationClass)
{
	...
	auto ret =
		__NtRoutine("NtQueryInformationFile", FileHandle, IoStatusBlock, FileInformation, Length, FileInformationClass);
	return ret;
}

NTSTATUS h_ZwQueryValueKey(HANDLE					   KeyHandle,
...
NTSTATUS h_RtlWriteRegistryValue(ULONG	RelativeTo,
								 PCWSTR Path,
								 PCWSTR ValueName,
								 ULONG	ValueType,
								 PVOID	ValueData,
...
NTSTATUS h_ObOpenObjectByPointer(PVOID		 Object,
...
NTSTATUS h_NtQueryInformationProcess(HANDLE			  ProcessHandle,
									 ...
									 PULONG			  ReturnLength)
{
	...
	{  // self-check
		...
		auto ret = __NtRoutine("NtQueryInformationProcess",
							   ProcessHandle,
...
NTSTATUS h_SeQueryInformationToken(PACCESS_TOKEN		   Token,
...
NTSTATUS h_KdChangeOption(ULONG	 Option,
						  ULONG	 InBufferBytes,
						  PVOID	 InBuffer,
...
NTSTATUS h_KeWaitForSingleObject(PVOID			Object,
								 void*			WaitReason,
								 void*			WaitMode,
								 BOOLEAN		Alertable,
...
NTSTATUS h_KeWaitForMutextObject(PVOID			Object,
								 void*			WaitReason,
								 void*			WaitMode,
								 BOOLEAN		Alertable,
								 PLARGE_INTEGER Timeout)
...
NTSTATUS h_PsCreateSystemThread(PHANDLE			   ThreadHandle,
								ULONG			   DesiredAccess,
								OBJECT_ATTRIBUTES* ObjectAttributes,
...
NTSTATUS h_ObReferenceObjectByHandle(HANDLE		   handle,
									 ACCESS_MASK   DesiredAccess,
...
NTSTATUS h_KdSystemDebugControl(int						Command,
								PVOID					InputBuffer,
...
NTSTATUS h_ZwOpenSection(PHANDLE SectionHandle, ACCESS_MASK DesiredAccess, OBJECT_ATTRIBUTES* ObjectAttributes)
{
	...
	Logger::Log(
		"\tSection name : %ls, access : %llx, ret : %08x\n", ObjectAttributes->ObjectName->Buffer, DesiredAccess, ret);

	return ret;
}

NTSTATUS h_ZwOpenFile(PHANDLE					 FileHandle,
					  ACCESS_MASK				 DesiredAccess,
					  _OBJECT_ATTRIBUTES*		 ObjectAttributes,
					  PVOID /*PIO_STATUS_BLOCK*/ IoStatusBlock,
					  ULONG						 ShareAccess,
					  ULONG						 OpenOptions)
{
...
RamRange myRam[9] = {{0x1000, 0x57000},
					 {0x59000, 0x46000},
					 {0x100000, 0xb81b9000},
					 {0xb82f1000, 0x3b0000},
...
PVOID h_MmAllocateContiguousMemorySpecifyCache(SIZE_T	 NumberOfBytes,
...
#define ADDRESS_AND_SIZE_TO_SPAN_PAGES(_Va, _Size) \
	((ULONG)((((ULONG_PTR)(_Va) & (PAGE_SIZE - 1)) + (_Size) + (PAGE_SIZE - 1)) >> PAGE_SHIFT))
...
void MmInitializeMdl(PMDL MemoryDescriptorList, PVOID BaseVa, SIZE_T Length)
{
	...
	MemoryDescriptorList->Size =
		(USHORT)(sizeof(MDL) + (sizeof(PFN_NUMBER) * ADDRESS_AND_SIZE_TO_SPAN_PAGES(BaseVa, Length)));
...
NTSTATUS
h_KeWaitForMultipleObjects(ULONG		  Count,
						   PVOID		  Object[],
						   uint32_t		  WaitType,
...
{  // This should set the Event to non-signaled
...
NTSTATUS h_ExGetFirmwareEnvironmentVariable(PUNICODE_STRING VariableName,
											LPGUID			VendorGuid,
											PVOID			Value,
											PULONG			ValueLength,
											PULONG			Attributes)
...
NTSTATUS h_ZwDeviceIoControlFile(HANDLE FileHandle,
								 HANDLE Event,
								 PVOID	ApcRoutine,
								 PVOID	ApcContext,
								 PVOID	IoStatusBlock,
								 ...
								 ULONG	OutputBufferLength)
{
	auto ret = __NtRoutine("NtDeviceIoControlFile",
						   FileHandle,
						   Event,
						   ApcRoutine,
						   ApcContext,
...
NTSTATUS h_IoGetDeviceInterfaces(const GUID*	 InterfaceClassGuid,
								 ...
								 wchar_t**		 SymbolicLinkList)
{
	...
	*SymbolicLinkList = (wchar_t*)0;
	// wcscpy(*SymbolicLinkList,
	// L"\\??\\PCI#VEN_8086&DEV_15B8&SUBSYS_86721043&REV_00#3&11583659&0&FE#{cac88484-7515-4c03-82e6-71a87abac361}");
...
NTSTATUS h_MmCopyMemory(PVOID			TargetAddress,
...
NTSTATUS h_FltReadFile(void*		  InitiatingInstance,
					   void*		  FileObject,
					   PLARGE_INTEGER ByteOffset,
					   ULONG		  Length,
					   PVOID		  Buffer,
...
NTSTATUS h_FltQueryInformationFile(void*				  Instance,
								   void*				  FileObject,
								   PVOID				  FileInformation,
								   ULONG				  Length,
								   FILE_INFORMATION_CLASS FileInformationClass,
...
void h_ObDereferenceObject(void* obj)
{
	return;
}
