    ...
    for (int i = 0; i < LoadedModuleArray.size(); i++) {
        if (LoadedModuleArray[i]->name == mod_name) {
            ...
            for (auto section = sections.begin(); section != sections.end(); section++) {
                ...
                if ((rva >= sectionData.virtual_address) && (rva <= (sectionData.virtual_address + sectionData.virtual_size))) {
                    if (enable) {
                        Logger::Log("Unhooking r/w of %s of %s\n", sectionName.c_str(), LoadedModuleArray[i]->name.c_str());
                        VirtualProtect(LoadedModuleArray[i]->mapped_buffer + sectionData.virtual_address, sectionData.virtual_size, PAGE_READWRITE,
                            &oldProtect);
                        return true;
                    } else {
                        Logger::Log("Rehooking r/w %s of %s\n", sectionName.c_str(), LoadedModuleArray[i]->name.c_str());
                        VirtualProtect(LoadedModuleArray[i]->mapped_buffer + sectionData.virtual_address, sectionData.virtual_size, PAGE_NOACCESS,
                            &oldProtect);
                        return true;
                       ;
                    }
