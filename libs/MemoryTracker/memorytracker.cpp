...
static __declspec(align(0x1000)) unsigned char PreAllocatedMemory[64 * 1024 * 1024] = {
	0};	 // 64MB - 15360 variables trackable, should be enough.
...
bool MemoryTracker::AddMapping(uintptr_t GVA, size_t size, uintptr_t HVA)
{
	...
	uint64_t totalPage =
		(size / 4096) +
		((size % 4096) ? 1 : 0);  // No matter how small, a variable will always use 0x1000 memory for easier tracking.
